...
func queryServiceVersionOnWorker(req apiServiceVersionReq) (string, error) {
	workersMutex.Lock()
	defer workersMutex.Unlock()
	...
	queryServiceVersionReq := pb.QueryServiceVersionReq{
		ServiceName: req.ServiceName,
	}

	r, err := c.QueryServiceVersion(ctx, &queryServiceVersionReq)
	if err != nil {
		return "", err
	}

	return r.ServiceVersion, nil
}
