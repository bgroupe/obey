...
// "ServiceVersion  the version metadata of a given service"
...
func queryServiceVersion(serviceName string) (string, error) {
	jobsMutex.Lock()
	defer jobsMutex.Unlock()

	fullAddr := fmt.Sprintf("https://%s.stage.peek.com/version.json", serviceName)
	...
	// something dumb going on here with unmarshaling into struct
