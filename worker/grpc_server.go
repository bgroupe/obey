...
func (s *server) QueryServiceVersion(ctx context.Context, r *pb.QueryServiceVersionReq) (*pb.QueryServiceVersionRes, error) {
	serviceVersion, err := queryServiceVersion(r.ServiceName)
	if err != nil {
		return nil, err
	}

	res := pb.QueryServiceVersionRes{
